...
Class MainWindow
    ...
    Private selectedFormatName As String = "adpcm"
    ...
    Private Sub InitializeEmbeddedResources()
        Dim tempDir = System.IO.Path.Combine(System.IO.Path.GetTempPath(), "wav2brwav_temp")
        System.IO.Directory.CreateDirectory(tempDir)

        _tempExePath = System.IO.Path.Combine(tempDir, "nw4r_waveconv.exe")
        ExtractEmbeddedResource("nw4r_waveconv.exe", _tempExePath)
    ...
    Private Sub Window_Loaded(sender As Object, e As RoutedEventArgs)
        If Not System.IO.File.Exists(_tempExePath) Then
            MessageBox.Show("无法提取nw4r_waveconv.exe,请确保资源已正确嵌入.", "错误", MessageBoxButton.OK, MessageBoxImage.Error)
        ...
        For Each dllPath In _tempDllPaths
            If Not System.IO.File.Exists(dllPath) Then
                missingDlls.Add(System.IO.Path.GetFileName(dllPath))
            End If
        Next

        If missingDlls.Count > 0 Then
            MessageBox.Show($"无法提取以下DLL文件:{Environment.NewLine}{String.Join(Environment.NewLine, missingDlls)}",
    ...
    Private Sub rbADPCM_Checked(sender As Object, e As RoutedEventArgs)
        selectedFormat = "--adpcm"
        selectedFormatName = "adpcm"
    End Sub
    ...
    Private Sub btnProcess_Click(sender As Object, e As RoutedEventArgs)
        ...
        If Not System.IO.File.Exists(_tempExePath) Then
            MessageBox.Show("找不到nw4r_waveconv.exe,无法进行转换.", "错误", MessageBoxButton.OK, MessageBoxImage.Error)
            Return
        End If

        For Each dllPath In _tempDllPaths
            If Not System.IO.File.Exists(dllPath) Then
                MessageBox.Show($"找不到{System.IO.Path.GetFileName(dllPath)},无法进行转换.", "错误", MessageBoxButton.OK, MessageBoxImage.Error)
                Return
            End If
        Next
